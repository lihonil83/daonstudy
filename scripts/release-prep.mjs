...
async function main() {
  ...
  const readmeText = [
    ...
    '',
    '사용 방법:',
    '1. Windows: start-windows.bat 을 더블클릭합니다.',
    '2. 개발 환경이 있으면 저장소 루트에서 npm run release:open 을 실행해도 됩니다.',
    '3. 또는 로컬 정적 서버로 dist 폴더를 서빙한 뒤 브라우저로 엽니다.',
    '4. file:///.../dist/index.html 직접 열기는 Chrome 계열에서 빈 화면이 될 수 있습니다.',
    '5. 학습 기록은 브라우저 localStorage 에 저장됩니다.',
    '',
  ...
  await fs.writeFile(
    path.join(bundleDir, 'start-windows.bat'),
    [
      '@echo off',
      'setlocal',
      'set "PORT=4174"',
      ...
      'where py >nul 2>nul',
      'if %errorlevel%==0 (',
      '  start "daonstudy server" cmd /k py -m http.server %PORT%',
      '  timeout /t 2 >nul',
      ...
      'if %errorlevel%==0 (',
      '  start "daonstudy server" cmd /k python -m http.server %PORT%',
      '  timeout /t 2 >nul',
      '  start "" http://127.0.0.1:%PORT%/',
      '  exit /b 0',
      ')',
      ...
      'echo Surface Book에서는 Microsoft Store 또는 python.org에서 Python 3를 설치한 뒤 다시 실행해 주세요.',
      'pause',
      '',
    ].join('\r\n'),
    'utf8',
